...
parser.add_argument(
    "--in-dir", default="~/", help="folder where input files are located"
)
parser.add_argument("--out", help="path to output h5 archive")
parser.add_argument("--scale", type=int, default=2, help="scaling factor")
parser.add_argument(
    "--dimension", type=int, help="dimension of patches--use -1 for no patching"
)
parser.add_argument(
    "--stride", type=int, default=3200, help="stride when extracting patches"
)
parser.add_argument(
    "--interpolate",
    action="store_true",
...
parser.add_argument(
    "--sam", type=float, default=1.0, help="subsampling factor for the data"
)
...
def add_data(h5_file, inputfiles, args, save_examples=False):
    ...
    if save_examples:
        ...
        librosa.output.write_wav(
            "example-lr.wav", lr_patches[40][0], fs / args.scale, norm=False
        )
    ...
    if args.dimension is not -1:
        ...
        print(len(ID_list))
        pickle.dump(
            ID_list, open("ID_list_patches_" + str(d) + "_" + str(args.scale), "wb")
        )
    else:
        # pickle the data
