...
class DataSet(object):
    def __init__(
        self, datapoints, labels, fake_data=False, one_hot=False, dtype=dtypes.float32
    ):
        """Construct a DataSet.
        ...
        if fake_data:
            ...
            assert (
                datapoints.shape[0] == labels.shape[0]
            ), "datapoints.shape: %s labels.shape: %s" % (
                datapoints.shape,
                labels.shape,
            )
    ...
    def next_batch(self, batch_size, fake_data=False, shuffle=True):
        ...
        if start + batch_size > self._num_examples:
            ...
            return numpy.concatenate(
                (datapoints_rest_part, datapoints_new_part), axis=0
