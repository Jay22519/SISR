...
from tensorflow.python.framework import dtypes, op_def_registry, ops, tensor_shape
...
def _conv_linear(args, filter_size, num_features, bias, bias_start=0.0, scope=None):
    ...
    with tf.compat.v1.variable_scope(scope or "Conv"):
        ...
        if len(args) == 1:
            res = tf.nn.conv2d(
                input=args[0], filters=matrix, strides=[1, 1, 1, 1], padding="SAME"
            )
        else:
            res = tf.nn.conv2d(
                input=tf.concat(axis=3, values=args),
                filters=matrix,
                strides=[1, 1, 1, 1],
                padding="SAME",
            )
...
class ConvLSTMCell(rnn_cell_impl.RNNCell):
    ...
    def __init__(
        ...
        if conv_ndims != len(input_shape) - 1:
            raise ValueError(
                "Invalid input_shape {} for conv_ndims={}.".format(
...
def _conv(args, filter_size, num_features, bias, bias_start=0.0):
    ...
    for shape in shapes:
        ...
        if len(shape) != len(shapes[0]):
            raise ValueError(
                "Conv Linear expects all args to be of same Dimensiton: %s"
                % str(shapes)
            )
        else:
