...
class Model(object):
    ...
    def __init__(
        ...
        if from_ckpt:
            ...
            alpha = tf.compat.v1.placeholder(
                tf.float32, shape=(), name="alpha"
            )  # weight multiplier
            # save inputs
    ...
    def create_train_op(self, X, Y, alpha):
        ...
        optimizer_vars = [
            ...
            if "optimizer/" in v.name or "Adam" in v.name
        ]
    ...
    def create_objective(self, X, Y, opt_params):
        ...
        # compute l2 loss
        sqrt_l2_loss = tf.sqrt(
            tf.reduce_mean(input_tensor=(P - Y) ** 2 + 1e-6, axis=[1, 2])
        )
        sqrn_l2_norm = tf.sqrt(tf.reduce_mean(input_tensor=Y ** 2, axis=[1, 2]))
        snr = 20 * tf.math.log(sqrn_l2_norm / sqrt_l2_loss + 1e-8) / tf.math.log(10.0)
    ...
    def fit(
        ...
        while train_data.epochs_completed < n_epoch:
            ...
            if train_data.epochs_completed > epoch:
                ...
                print(
                    "Epoch {} of {} took {:.3f}s ({} minibatches)".format(
                        epoch,
                        n_epoch,
                ...
                print(
                    "  training l2_loss/segsnr/LSD:\t\t{:.6f}\t{:.6f}\t{:.6f}".format(
                        tr_l2_loss, tr_l2_snr, tr_lsd
                    )
                )
                print(
                    "  validation l2_loss/segsnr/LSD:\t\t{:.6f}\t{:.6f}\t{:.6f}".format(
                ...
                if (
                    calc_full_snr
                    and train_data.epochs_completed % 1 == 0
                    and grocery == "false"
                ):
                    s1 = ""
                    s2 = ""
    ...
    def load_batch(self, batch, alpha=1, train=True):
        ...
        g = tf.compat.v1.get_default_graph()
        k_tensors = [
            n for n in g.as_graph_def().node if "keras_learning_phase" in n.name
        ]
        if k_tensors:
