...
def make_parser():
    ...
    train_parser.add_argument(
        "--pool_size", type=int, default=4, help="size of pooling window"
    )
...
def train(args):
    ...
    if args.grocery == "false":
        ...
        X_train = pickle.load(
            open("../data/grocery/grocery/grocery-train-data" + args.train)
        )
        Y_train = pickle.load(
            open("../data/grocery/grocery/grocery-train-label" + args.train)
        )
        X_val = pickle.load(
            open("../data/grocery/grocery/grocery-test-data_" + args.train)
        )
        Y_val = pickle.load(
            open("../data/grocery/grocery/grocery-test-label" + args.train)
        )
    ...
    if args.piano == "true":
        X_train = np.reshape(
            X_train, [X_train.shape[0], X_val.shape[2], X_val.shape[1]]
        )
        Y_train = np.reshape(
            Y_train, [Y_train.shape[0], Y_val.shape[2], Y_val.shape[1]]
        )
...
def eval(args):
    # load model
    model = get_model(
        args, 0, args.r, from_ckpt=True, train=False, grocery=args.grocery
    )
            ...
            for line in f:
                try:
                    ...
                    if args.speaker == "single":
                        upsample_wav(
                            "../data/vctk/VCTK-Corpus/wav48/p225/" + line.strip(),
                            args,
                            model,
                        )
                    else:
                        upsample_wav(
                            "../data/vctk/VCTK-Corpus/" + line.strip(), args, model
                        )
                except EOFError:
