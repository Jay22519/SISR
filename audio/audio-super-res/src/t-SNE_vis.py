...
print("loading...")
speakerInfo = np.loadtxt(
    "../data/vctk/VCTK-Corpus/speaker-info.txt",
    dtype={
        "names": ("ID", "AGE", "GENDER", "ACCENT", "REGION"),
...
ids = np.load(
    "../data/vctk/multispeaker/ID_list"
)  # this associates each audio sample with the id of its speaker
# i.e., the id on line 1 is the id of the sample in row 1 of the datafile

print("loaded id list")
data = pickle.load(
    open(
        "../data/vctk/multispeaker/full-data-vctk-multispeaker-interp-val.4.16000.-1.8192.0.25"
    )
)
...
with tf.compat.v1.Session() as sess:
    ...
    saver = tf.compat.v1.train.import_meta_graph(
        "./full-snr-multispeaker_audiohybrid2.lr0.00300.1.g4.b32.d8192.r4.lr0.000300.1.g4.b32/model.ckpt-41761.meta"
    )
    graph = tf.compat.v1.get_default_graph()
    saver.restore(
        sess,
        tf.train.latest_checkpoint(
            "./full-snr-multispeaker_audiohybrid2.lr0.00300.1.g4.b32.d8192.r4.lr0.000300.1.g4.b32"
        ),
    )
    ...
    for i in tqdm(list(range(0, len(data)))):
        ...
        k_tensors = [
            n for n in graph.as_graph_def().node if "keras_learning_phase" in n.name
        ]
    ...
    for layer, acts in tqdm(iter(maps.items())):
        ...
        pts = [
            plt.Line2D((0, 1), (0, 0), color=c, marker="o", linestyle="")
            for c in list(genders.values())
        ]
